...
public class OopsieAnnotatedTypeFactory
    ...
    private final OopsieTypeCache oopsieTypeCache;
    ...
    public OopsieAnnotatedTypeFactory(BaseTypeChecker checker) {
        ...
        oopsieTypeCache = new OopsieTypeCache(this); // todo review initialization warning
    ...
    private void initSchemaInfo(
            @UnderInitialization OopsieAnnotatedTypeFactory this, BaseTypeChecker checker) {
    ...
    private final class OopsieQualifierHierarchy extends MostlyNoElementQualifierHierarchy {
        ...
        private OopsieQualifierHierarchy(
        ...
        protected boolean isSubtypeWithElements(
            ...
            if (subKind == SQL_KIND && superKind == SQL_KIND) {
                List<OopsieType> subInTypes = oopsieTypeCache.getInTypes(subAnno);
                List<OopsieType> subOutTypes = oopsieTypeCache.getOutTypes(subAnno);
                List<OopsieType> superInTypes = oopsieTypeCache.getInTypes(superAnno);
                List<OopsieType> superOutTypes = oopsieTypeCache.getOutTypes(superAnno);
         ...
         */
        private boolean outIsSubtype(List<OopsieType> subOutTypes, List<OopsieType> superOutTypes) {
            ...
            for (int i = 0; i < subOutTypes.size(); i++) {
                OopsieType sub = subOutTypes.get(i);
                OopsieType sup = superOutTypes.get(i);
        ...
        private boolean inIsSubtype(List<OopsieType> subIn, List<OopsieType> superIn) {
        ...
        protected AnnotationMirror leastUpperBoundWithElements(
            ...
            if (qualifierKind1 == SQL_KIND && qualifierKind2 == SQL_KIND) {
                if (a1 == a2) {
                    ...
                    List<OopsieType> in1 = oopsieTypeCache.getInTypes(a1);
                    List<OopsieType> in2 = oopsieTypeCache.getInTypes(a2);
                    List<OopsieType> out1 = oopsieTypeCache.getOutTypes(a1);
                    List<OopsieType> out2 = oopsieTypeCache.getOutTypes(a2);
                    ...
                    for (int i = 0; i < maxLubOutSize; i++) {
                        OopsieType out1Type = out1.get(i);
                        OopsieType out2Type = out2.get(i);
    ...
    protected AnnotationMirror annotateStatement(
        ...
        if (!annos.isEmpty() && !annos.stream().allMatch(a -> sqlAnnotationsEqual(a, firstAnno))) {
            ...
            logger.simpleStatementEntry(
                    OopsieLogEntryKind.CANNOT_DETERMINE_STATEMENT_STRING,
                    getRoot(),
    ...
    private @Nullable AnnotationMirror buildSqlAnnotation(
        ...
        try {
            ...
            if (warnAndLog) {
                logger.simpleStatementEntry(
                        OopsieLogEntryKind.USING_FALLBACK,
                        getRoot(),
                        getStartPosition(tree),
                        calciteException.getMessage(),
                        isPreparedStatement);
    ...
    private @Nullable List<String> retrieveStringValue(
        ...
        if (checker.getBooleanOption("enableSqlStringHeuristic")) { // todo: remove this option?
            checker.reportWarning(
                    ...
                    values.toString());
            logger.simpleStatementEntry(
                    OopsieLogEntryKind.USING_SQL_STRING_HEURISTIC,
                    getRoot(),
                    getStartPosition(stringExpression),
                    null,
                    isPreparedStatement);
