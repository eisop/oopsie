...
public class OopsieVisitor extends BaseTypeVisitor<OopsieAnnotatedTypeFactory> {
    ...
    protected final ExecutableElement sqlFileElement =
            TreeUtils.getMethod("io.github.eisop.oopsie.qual.Sql", "file", 0, processingEnv);
    protected final ExecutableElement sqlLineElement =
            TreeUtils.getMethod("io.github.eisop.oopsie.qual.Sql", "line", 0, processingEnv);
    protected final ExecutableElement sqlColumnElement =
            TreeUtils.getMethod("io.github.eisop.oopsie.qual.Sql", "column", 0, processingEnv);
    ...
    public OopsieVisitor(BaseTypeChecker checker) {
        super(checker);
    ...
    @Override
    public OopsieAnnotatedTypeFactory createTypeFactory() {
        return new OopsieAnnotatedTypeFactory(checker);
    }
    ...
    private void checkParameterType(
        ...
        OopsieCheckResult result = typeMapping.checkCall(methodName, jdbcType);
        if (result.getKind() == OopsieCheckResultKind.ERROR) {
            ...
            logError(
                    ...
                    sqlAnnotation);
        } else if (result.getKind() == OopsieCheckResultKind.WARNING) {
    ...
    private void checkGetResultByName(MethodInvocationTree tree, ExecutableElement method) {
        ...
        if (receiverType.hasAnnotation(Sql.class)) {
            ...
            List<String> stringValues =
                    OopsieUtils.retrieveStringValues(
    ...
    private void checkGetResult(
        ...
        if (index >= out.size()) {
            ...
            OopsieCheckResult result = typeMapping.checkCall(methodName, out.get(index));
            if (result.getKind() == OopsieCheckResultKind.ERROR) {
    ...
    static @Nullable String getName(String annotationString) {
        // todo improve: e.g. with class for oopsie type
